...
export class AccountingResolver {
    ...
    async export(@Args('input', { type: () => ExportArgsDTO }) input: ExportArgsDTO) {
        // Construct FindManyOptions based on input filters, sorting, relations, etc.
        // Fetch data from the database using TypeORM and specified criteria
        // Apply certain modifications to the fetched data based on conditions (like demo mode)
        // If export type is CSV, convert data to CSV format and write to a file
